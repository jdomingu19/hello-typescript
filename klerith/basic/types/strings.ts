...
(() => {
  ...
  console.log({ myString }); // { myString: 'Hello, TypeScript!' }
  console.log(typeof myString); // string

  // 2. Explicit string type annotation
  // - Declared as type string, not literal
  // - On editor hover => const myString2: string
  const myString2: string = "Hello, Node.js!";

  console.log({ myString2 }); // { myString: 'Hello, Node.js!' }
  ...
  const myString3: string = "He's often called @jdomingu19";

  console.log({ myString3 }); // { myString3: "He's often called @jdomingu19" }
  ...
  const myString4: string = 'He"s often called @jdomingu19';

  console.log({ myString4 }); // { myString4: 'He"s often called @jdomingu19' }
  ...
  const myString5: string = 'He\'s often called "@jdomingu19"';

  console.log({ myString5 }); // { myString5: `He's often called "@jdomingu19"` }
  ...
  const myString6: string = `He's often called "@jdomingu19"`;

  console.log({ myString6 }); // { myString6: `He's often called "@jdomingu19"` }
  ...
  const myString7: any = "abc";

  console.log({ myString7 }); // { myString7: `He's often called "@jdomingu19"` }
  ...
  console.log(`He's often called "${myAlias}"`); // He's often called "@jdomingu19"
  ...
  let x = myNumber.toString();

  console.log({ x }); // { x: '42' }
  console.log(typeof x); // string

  console.log(
    `${myNumber} and ${myNumber.toString()} in templated string are the same`,
  ); // 42 and 42 in templated string are the same

  // 11. String length property
  // - On editor hover => const myName: string
  const myName: string = "Jesús";

  console.log({ myName }); // { myName: 'Jesús' }
  ...
  console.log(myName[5]?.toLocaleUpperCase()); // undefined

  // 14. Default message when index is out of range
  // - On editor hover => let defaultMessage: string
  let defaultMessage = `String '${myName}' is indexed from 0 to ${myName.length - 1}`;
  console.log(myName[5]?.toLocaleUpperCase() || defaultMessage); // String 'Jesús' is indexed from 0 to 4
})();
